...
- (void)viewDidLoad {
    ...
    _messageView.autoresizingMask = UIViewAutoresizingFlexibleWidth-8 | UIViewAutoresizingFlexibleHeight-8;
    ...
    if (![[NSUserDefaults standardUserDefaults] boolForKey:@"FetchFullMessageEnabled"]) {
        [_messageView setDelegate:(id)self];
        [_messageView setFolder:_folder];
        [_messageView setMessage:_message];
        
        
        NSLog(@"To Array %@",  _message.header.to);
        NSLog(@"From Array %@",  _message.header.replyTo);
        NSLog(@"BCC Array %@",  _message.header.bcc);
        NSLog(@"CC Array %@",  _message.header.cc);
    }
    else {
